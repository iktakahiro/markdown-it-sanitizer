...
export function sanitizer_plugin(md, options: SanitizerOptions = {}) {
    ...
    const patternLinkOpen = '<a\\s([^<>]*href="[^"<>]*"[^<>]*)\\s?>'
    const regexpLinkOpen = RegExp(patternLinkOpen, "i")
    // <img src="url" alt=""(optional) title=""(optional)>
    const patternImage = '<img\\s([^<>]*src="[^"<>]*"[^<>]*)\\s?\\/?>'
    const regexpImage = RegExp(patternImage, "i")
    const regexpImageProtocols = /^(?:https?:)?\/\//i
    const regexpLinkProtocols = /^(?:https?:\/\/|ftp:\/\/|\/\/|mailto:|xmpp:)/i

    options = options ? options : {}
    const removeUnknown = (typeof options.removeUnknown !== "undefined") ? options.removeUnknown : false
    const removeUnbalanced = (typeof options.removeUnbalanced !== "undefined") ? options.removeUnbalanced : false
    const imageClass = (typeof options.imageClass !== "undefined") ? options.imageClass : ""

    let runBalancer = false
    let j: number

    const allowedTags = ["a", "b", "blockquote", "code", "em", "h1", "h2", "h3", "h4", "h5",
        "h6", "li", "ol", "p", "pre", "s", "sub", "sup", "strong", "ul"]
    const openTagCount = new Array(allowedTags.length)
    const removeTag = new Array(allowedTags.length)
    for (j = 0; j < allowedTags.length; j++) { openTagCount[j] = 0 }
    for (j = 0; j < allowedTags.length; j++) { removeTag[j] = false }

    function getUrl(link): string | null {
    ...
    function replaceUnknownTags(str: string): string {
        /*
    ...
    function balance(state) {
        ...
        function replaceUnbalancedTag(str: string, tagname: string): string {
            ...
            if (removeUnbalanced === true) {
                ...
                str = str.replace(openingRegexp, m => {
                    return escapeHtml(m)
                })
                str = str.replace(closingRegexp, m => {
                    return escapeHtml(m)
                })
            }
            return str
        ...
        function replaceAllUnbalancedTags(str: string): string {
            let i: number
