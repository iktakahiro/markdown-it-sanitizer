...
function sanitizer_plugin(md, options) {
    ...
    var patternLinkOpen = '<a\\s([^<>]*href="[^"<>]*"[^<>]*)\\s?>';
    ...
    var regexpImageProtocols = /^(?:https?:)?\/\//i;
    var regexpLinkProtocols = /^(?:https?:\/\/|ftp:\/\/|\/\/|mailto:|xmpp:)/i;
    ...
    function balance(state) {
        ...
        function replaceUnbalancedTag(str, tagname) {
            ...
            if (removeUnbalanced === true) {
                ...
                str = str.replace(openingRegexp, function (m) {
                    return escapeHtml(m);
                });
                str = str.replace(closingRegexp, function (m) {
                    return escapeHtml(m);
                });
            }
            return str;
...
exports.sanitizer_plugin = sanitizer_plugin;
